...
//         <Link to="/recipes" className="text-[#034694] hover:text-[#012147] mt-2 inline-block">
//           Back to Recipes
//         </Link>
//       </div>
//     );
...
//                 <span className="text-[#034694] text-xl font-bold">₹{recipe.totalPrice * servings}</span>
//               </div>
...
//         <button
//           className="bg-[#034694] text-white text-lg font-bold px-8 py-2 rounded-lg shadow hover:bg-[#012147] transition"
//           onClick={addToCart}
//         >
//           Add to Cart
//         </button>
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          recipeResponse = await axios.get(`https://localhost:3000/api/recipes/${id}`);
        }
        setRecipe(recipeResponse.data);
      } catch (error) {
        console.error("Error fetching data:", error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-10 items-start">
        {/* Image Section */}
        <div className="w-full md:w-1/2 rounded-xl overflow-hidden shadow-lg border border-gray-200">
          <img
            src={recipe.recipeImage ? `/api/${recipe.recipeImage}` : "/src/assets/images/meal.png"}
            alt={recipe.title}
        ...
        {/* Info Card */}
        <div className="w-full md:w-1/2 bg-white shadow-xl rounded-xl p-8 space-y-6 border border-gray-200">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-800">{recipe.title}</h1>
              <div className="flex items-center gap-3 mt-2 text-sm text-gray-600 font-medium">
                <FaClock className="text-[#034694]" />
                <span>{recipe.prepTime} mins</span>
                <span className="text-[#034694] font-semibold text-lg">₹{recipe.totalPrice * servings}</span>
              </div>
            </div>
            {user && <SavedRecipes recipeId={recipe._id} />}
          </div>
          ...
          {/* Servings */}
          <div>
            <span className="font-semibold text-gray-800 block mb-1">Adjust Servings</span>
            <div className="flex items-center gap-4">
              <button
                className="w-10 h-10 flex items-center justify-center bg-gray-200 rounded-full hover:bg-green-100"
                onClick={() => setServings(s => Math.max(1, s - 1))}
              >
                <FaMinus />
              </button>
          ...
          <button
            className="w-full bg-[#034694] text-white py-3 rounded-lg font-bold text-lg hover:bg-[#012147] transition"
            onClick={addToCart}
          >
            Add to Cart
          </button>
      ...
      {/* Ingredients & Steps */}
      <div className="max-w-6xl mx-auto mt-14 grid grid-cols-1 lg:grid-cols-2 gap-12">
        {/* Ingredients */}
        <div className="bg-white rounded-xl shadow-md p-8 border border-gray-200">
          <h2 className="text-2xl font-bold mb-4 text-[#034694]">Ingredients</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 text-gray-800">
            {getIngredientColumns(recipe.ingredients).map((col, i) => (
        ...
        {/* Steps */}
        <div className="bg-white rounded-xl shadow-md p-8 border border-gray-200">
          <h2 className="text-2xl font-bold mb-4 text-[#034694]">Cooking Instructions</h2>
      ...
      <div className="text-center mt-12">
        <Link
          to="/recipes"
          className="inline-block text-[#034694] text-lg font-semibold hover:underline"
        >
          ← Back to Recipes
        </Link>
      </div>
